...
import { LucideAngularModule, Store, Map, PackageSearch, Plus, Trash2, PencilRuler } from 'lucide-angular';
...
export class StoreConfigurationPageComponent implements OnInit {
  ...
  submitAddZone() {
    ...
    const zone: Omit<StoreZoneLayout, 'id'> = {
  ...
  onSaveLayout(zones: StoreZoneLayout[]) {
    this.store.saveZoneLayout(zones);
    this.editMode.set(false);
  }

  onCancelEdit() {
    this.store.loadStoreData(); // reload from API to discard
    this.editMode.set(false);
  }
}
